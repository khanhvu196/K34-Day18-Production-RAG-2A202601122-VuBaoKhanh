...
def evaluate_ragas(questions: list[str], answers: list[str],
    ...
    try:
        ...
        if "googleapis.com" in base_url:
            llm = ChatOpenAI(model="gemini-3.6-flash", base_url=base_url)
            embeddings = OpenAIEmbeddings(model="text-embedding-004", base_url=base_url)
        else:
        ...
        df = result.to_pandas().fillna(0.0)
