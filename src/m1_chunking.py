...
def chunk_semantic(text: str, threshold: float = SEMANTIC_THRESHOLD,
    ...
    return [Chunk(text=c, metadata={**metadata, "strategy": "semantic"}) for c in chunks]
    ...
    for part in sections:
        if re.match(r'^#{1,3}\s+.+$', part):
            if current_content.strip():
                ...
                chunks.append(Chunk(text=full_text, metadata={**metadata, "section": current_header.strip() if current_header else "None", "strategy": "structure"}))
